...
async function summarize(text) {
    const prompt = createPrompt(
        `Please summarize the key points from the transcript by extracting the essential information, removing unnecessary details, and refraining from introducing any external sources or speculative content. Focus solely on the core ideas discussed in the text without mentioning the speaker or their beliefs.`
    )
    return await transform(text, prompt)
}

async function outline(text) {
    const prompt = createPrompt(`write an outline`)
    return await transform(text, prompt)
}

export { topics, summarize, outline }
